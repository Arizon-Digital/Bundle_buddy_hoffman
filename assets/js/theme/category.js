...
export default class Category extends CatalogPage {
    ...
    onReady() {
        ...
        let cardCompareList = document.querySelectorAll(".card .card-figcaption-bottom .card-compare");
        for(let item of cardCompareList) {
    ...
    /* Check filter Devide */
    checkFilterDevide() {
        let filterDesktop = $("#faceted-search-container.filter-desktop #facetedSearch");
        let filterMobile = $("#faceted-search-container.custom-show-on-mobile nav");

        if($(window).width() < 768) {
            filterDesktop.appendTo(filterMobile);
        }

        let filterButton = $('#facet-range-form .button');
